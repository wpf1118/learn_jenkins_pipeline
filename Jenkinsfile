            ...
            steps{
                ...
                script{
                    if (env.BRANCH_NAME == 'build/dev' || env.BRANCH_NAME == 'build/test' || env.TAG_NAME){
                        ...
                        # build docker image
                        sudo docker pull takatost/nginx-php-fpm:latest
                        sudo docker build . -f dockerfiles/Dockerfile -t ${dockerImage}
                        ...
                        # build docker image
                        sudo docker build . -f dockerfiles/docs/Dockerfile -t ${dockerImage}
                    ...
                    if ("${env.needDeploy}".contains("Yes")) {
                        ...
                        withCredentials([kubeconfigContent(credentialsId: 'dev_test-vchangyi-com', variable: 'KUBECONFIG_CONTENT')]) {
                            sh '''
                                ...
                                whoami
                                kubectl set image deployment/console-api console-api=${onlineImage} --namespace=${k8sNamespace} --kubeconfig=kubeconfig
                                rm kubeconfig
                            '''
                        }
                        echo "部署服务器完成。。。"
